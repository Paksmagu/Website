...
class Canvas {
    ...
    initKeyboard() {
        ...
        const whiteKeyBorderOffset = constants.whiteKeysInOctave * constants.numberOfOctaves * constants.borderThickness;
    ...
    drawWhiteKeys(octaveIndex) {
        let whiteKeyBorderOffset = constants.whiteKeysInOctave * constants.borderThickness * octaveIndex; //Take into account the border thickness of the white keys
        const octaveOffset = (octaveIndex * constants.whiteKeysInOctave * this.whiteKeyWidth) + this.whiteKeyWidth; //when a new octave starts increase the width
        for (let keyInOctave = 0; keyInOctave < constants.whiteKeysInOctave; keyInOctave++) {
    ...
    drawBlackKeys(octaveIndex) {
        ...
        const blackKeyStartPlace = this.whiteKeyWidth + constants.halfBorderThickness - (this.blackKeyWidth / 2) + this.whiteKeyWidth; //Between 2 white notes
        const octaveOffset = octaveIndex * (this.whiteKeyWidth + constants.borderThickness) * constants.whiteKeysInOctave; //when a new octave starts increase the width
        for (let keyInOctave = 0; keyInOctave < constants.blackKeysInOctave; keyInOctave++) {
